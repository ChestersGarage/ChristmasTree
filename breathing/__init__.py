...
class Scene(object):
    ...
    def map_sine_to_pixel_sequence(self, sine_map, pixel_color):
        """
    ...
    def breathing(self, pixel_color):
        """
        ...
        pixel_sequence = self.map_sine_to_pixel_sequence(sine_map, pixel_color)
        return pixel_sequence
    ...
    def led_values(self):
        ...
        while pixel < self._pixel_count:
            ...
            if self._sequence_counter[pixel] == len(self._string_sequence[pixel]):
                self._string_sequence[pixel] = self.breathing(pixel_color)
                self._sequence_counter[pixel] = 0
            color += 1
            if color == self._palette_size:
                color = 0
            pixel += 1
        return next_frame
